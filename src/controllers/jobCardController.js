...
export const createVRS = async (req, res) => {
    try {
        const { make, model, plateNumber, vin, category, customerName, customerEmail, customerPhone } = req.body;
...
export const updateJobStatus = async (req, res) => {
    try {
        ...
        const job = await JobCard.findByIdAndUpdate(req.params.id, { status }, { new: true });
        if (!job)
            return res.status(404).json({ message: 'Job Card not found' });
        res.json(job);
    }
...
//# sourceMappingURL=jobCardController.js.map